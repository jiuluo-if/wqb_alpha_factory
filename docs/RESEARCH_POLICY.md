...
- BRAIN 是 Simulation、Alpha、指标、检查项、聚合、PnL 和相关性的事实源。
...
```text
列出原始 datasets/datafields 与 operators → Agent 选择字段/假设
→ 按需生成显式选定/审阅过的 SimulationSpecs → 批量/Multi 模拟
→ 读取 live Alpha 证据 → AI 比较、解释并创建下一份 spec
```

执行成功不等于经济机制得到支持。缺失或未确认的远端证据保持 `UNKNOWN`/`UNAVAILABLE`；缓存必须标明来源和新鲜度，不能覆盖实时 response。
...
所有 Simulation 经过 `research_api → SimulationGateway → Simulator → WQBClient`。`SUBMIT_UNKNOWN` 不得自动重 POST，已知 progress URL 只能轮询原任务。Alpha 提交始终 `MANUAL_ONLY`；颜色 metadata PATCH 与提交分离；颜色只能由同一远端 evidence snapshot 的 variant family 经 AI 显式 assignment 产生，同时保留 strict structural key，且同步必须消费 review 后的 exact plan 并通过 stale/readback 检查。
