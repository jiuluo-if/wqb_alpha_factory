...
颜色和 family CI 使用 synthetic evidence 和 mock setter；质量字段只能作为文本状态，不能自动产生颜色。`observed_variant_count` 只代表当前 evidence snapshot/window 的可见成员数，不代表完整 trial count。每个生产 Python 模块必须在 `scripts/run_targeted_tests.py` 的 `DIRECT_TESTS` 中有显式路由；映射不存在或过期都 fail closed。
