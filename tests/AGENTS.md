...
代码变更必须运行直接相关的定向测试；本地迭代禁止运行全仓测试：
...
不得使用 `unittest discover`、无选择器的 `pytest` 或 coverage 驱动的全量测试。测试映射缺失时先补充 `scripts/run_targeted_tests.py` 的显式映射。交付前最终门仍按根目录 `AGENTS.md` 执行全量 unittest 与 privacy gate。
