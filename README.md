...
- 可选 Probe/template：`list_templates()`、`inspect_template()`、`generate_probes(fields=..., template_ids=..., count=...)`。字段、模板和候选数都由 Agent 显式选择；每次最多传入 100 个字段、100 个模板 ID，并请求生成 100 个候选，属于本地资源硬上限。
...
```text
list raw datasets/datafields → Agent selects fields and hypothesis
→ optionally render explicitly selected templates → simulate → read live evidence
